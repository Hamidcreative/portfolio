...
define("BACKUP_DIR", 'db_backups'); // Comment this line to use same script's directory ('.')C:/Users/hc/Dropbox
...
class Databasebackup {
    ...
    public function __construct(array $config = array()) {
        ...
        $this->backupFile      = 'bgi_'.$this->dbName.'_database_'.date("Y_m_d_H_i_s", time()).'.sql';
        $this->gzipBackupFile  = defined('GZIP_BACKUP_FILE') ? GZIP_BACKUP_FILE : true;
        $this->batchSize       = defined('BATCH_SIZE') ? BATCH_SIZE : 1000; // default 1000 rows
        $this->output          = '';

        $result = $this->backupTables(TABLES, BACKUP_DIR);
        $this->obfPrint('Backup result: ' . $result, 1);
    }
